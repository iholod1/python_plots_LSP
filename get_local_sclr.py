#!/usr/bin/env python
# scan all sclr file in directory and write box-averaged value 
# of probe quantity into local_probe.out file
# I. Holod, 07/26/17

import sys, argparse, os, os.path
from mpi4py import MPI
from return_local import return_local_sclr, return_local_ezden
import numpy as np
...
probe = int(arg.p) if arg.p != None else 1;
...
for i in myInd:
  (tdum,vdum,vname,vunit)=return_local_sclr(dirname,fnames[i],probe,x0,x1,z0,z1);
  if probe==100: (tdum,vdum,vname,vunit)=return_local_ezden(dirname,fnames[i],x0,x1,z0,z1);
  dat[i,0]=tdum
  dat[i,1]=vdum
